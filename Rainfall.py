...
raindf = pd.read_csv('rainfall_india.csv')
...
rain_past.to_csv('rainfall_past.csv')
raindf.to_csv('rainfall_india_processed.csv')
...
for i, color in zip(range(n_classes), colors):
    pp.plot(fpr[i], tpr[i], color=color,label='ROC curve of class {0} (area = {1:0.2f})'''.format(i, roc_auc[i]))
...
pp.title('Receiver operating characteristic for multi-class data')
...
svm_linear_pred = svm_linear.predict(Xtrain)
svm_poly_pred = svm_rad.predict(Xtrain)

print("Classification accuracy using SVM linear classifier is: {}".format(accuracy_score(ytrain, svm_linear_pred) * 100))
print("Classification accuracy using SVM radial classifier is: {}".format(accuracy_score(ytrain, svm_poly_pred) * 100))
...
plot_decision_regions(Xtreduced, yt, svm_linear)
